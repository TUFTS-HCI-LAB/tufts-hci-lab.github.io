...
module.exports = (options, ctx) => {
  ...
  return {
    ...
    async ready () {
      ...
      // Set how many pages needed for each key (root/people/post, .etc)
      const setPage = map => {
      ...
      // According to the number of pages needed for each key (root/people/post, .etc), add the corresponding pages
      ...
      // console.log('listMap before sort', listMap)
      ...
      pages.sort(postsSorter).forEach(({
        ...
        // Classify the items to display on home page
        display === 'home'
      ...
      // console.log('listMap after sort, before set page', listMap)

      ctx.tagMap = setPage(tagMap)
      ctx.categoryMap = setPage(categoryMap)
      ctx.listMap = setPage(listMap)

      // console.log('listMap after set page', listMap)
      // console.log(addPages(ctx.listMap, 'listName'))
    ...
    extendPageData($page) {
      ...
      // console.log($page.frontmatter.layout)
      ...
      const setLink = (dir, link) => {
        link
          ...
          && ($page.frontmatter.permalink = link), console.log('link = ',link)
      }

      // If there is no layout assigned in yaml, need to assign it now
      if (!layout){
        //  it's html page (single page manually generated)
        //  then set the layout to be 'Page'
        if (regularPath.includes('.html'))
          {
            $page.frontmatter.layout = postLayout
          }
        // the page is automatically generated
        else {
          // it's home page
          if ($page._meta && $page._meta.listName && $page._meta.listName == aliasesRoot) {
          // it's other pages
          $page.frontmatter.layout = 'LayoutHome'
          }
          else {
      ...
      // if ($page._meta && $page._meta.listName && $page._meta.listName == aliasesRoot){
      //   console.log($page._meta)
      // }
      console.log('layout = ',$page.frontmatter.layout)

      // Set permalink for all folders in postsDir
